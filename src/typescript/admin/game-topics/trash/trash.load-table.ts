import { GameTopicData, ICardTheme, IGameTopic } from "../../../types/Api.js";
import { Button, Tag } from "../../admin.helpers.js";
import {
	fetchGameTopics,
	fetchGameTopicsDeleted,
	mapDeleteButton,
	mapEditButton,
...
const generateTableRow = ({ _id, themeName, themeData, price, played, isVip, isDeleted }: IGameTopic, isHaveViewButton: boolean) => {
	return `<tr>
                <td>${themeName}</td>
...
const mapGameTopicData = (listGameTopics: IGameTopic[], isHaveViewButton: boolean) => {
	return listGameTopics.map((gameTopicData) => generateTableRow(gameTopicData, isHaveViewButton)).join("");
};

export const loadTable = async (isHaveViewButton: boolean = true) => {
